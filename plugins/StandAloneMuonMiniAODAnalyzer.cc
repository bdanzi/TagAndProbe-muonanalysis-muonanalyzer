  ...
  for (const auto& mu : *muons) {
    ...
    for (const auto& trk : nocut_tracks) {
     ...
     isAssoc = false;
        ...
        if ((trk.pt() <= minpt_trkSA_) && (abs(trk.eta()) <= 1. || trk.p() <= 2.)) continue;
        ...
        for (const auto& tag : tag_muon_ttrack) {
            float mass_tagtrack = (tag.first.p4() + trk.p4()).M();
            if (mass_tagtrack >= 70 && mass_tagtrack <= 115) { isZmass = true; break; }
        }
      ...
      for (const auto& probe : tracks) {
        ...
        if (it != trk_muon_map.first.end()) {
          ...
          FillProbeBranches<pat::Muon, pat::PackedCandidate>(
              muons->at(trk_muon_map.second[idx]), tracks, nt, true, *pv);
